...
class WellnessAutoRegressiveDataset(Dataset):
  ...
  def __init__(self,
               file_path = "C:\\sqlite\\mysql\\code\\AI\\FINAL_project\\dialogLM\\dialogLM\\NewData\\wellness_dialog_for_autoregressive2.txt",
               n_ctx = 1024
               ):
...
class WellnessTextClassificationDataset(Dataset):
  ...
  def __init__(self,
               file_path = "C:\\sqlite\\mysql\\code\\AI\\FINAL_project\\dialogLM\\dialogLM\\NewData\\wellness_dialog_for_text_classification.txt",
               num_label = 359,
               device = 'cpu',
    ...
    self.device = device
    self.data =[]
    self.tokenizer = tokenizer if tokenizer is not None else BertTokenizer.from_pretrained('monologg/kobert')
