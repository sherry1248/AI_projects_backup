...
def test_handle_chat_ui_returns_messages_format(monkeypatch):
    ...
    history, cleared_message, pipeline, summary, status = asyncio.run(
...
def test_chatbot_has_fixed_scrollable_height_and_next_step_buttons():
    source = inspect.getsource(demo_app.create_demo)

    assert "height=460" in source
    assert "상담 후 필요하면 다음 단계" in source
    assert "상태 체크하기" in source
    assert "감정일기 쓰기" in source
    assert "마음정리 보고서 보기" in source
    assert "전문가 상담 연결" in source
