...
class TestCrisisHandler:
    ...
    def test_immediate_danger_prioritizes_korean_numbers(self, handler, crisis_assessment):
        """Test that Korean emergency numbers are first in immediate danger messages."""
        response = handler.get_response(crisis_assessment, crisis_type="immediate_danger")

        assert response.message.startswith("109 / 119 / 112")
        if "988" in response.message:
            assert response.message.index("109") < response.message.index("988")
        if "911" in response.message:
            assert response.message.index("109") < response.message.index("911")
...
class TestAuditLogger:
    ...
    def test_audit_log_file_redacts_pattern_reference(self, logger):
        """Test that persisted audit logs do not store raw matched patterns."""
        raw_input = "I want to kill myself"

        logger.log_safety_check(
            session_id="test-session",
            risk_level="high",
            is_safe=False,
            matched_pattern=raw_input,
            action_taken="ESCALATE_IMMEDIATELY"
        )

        log_files = list(Path(logger.config.log_dir).glob("*.jsonl"))
...
class TestIntegration:
    ...
    def test_risk_stage_remains_in_expected_set(self):
        """Test that risk stages are limited to the supported Korean labels."""
        checker = RiskChecker()

        scenarios = [
            (AnalysisResult(risk_level=RiskLevel.LOW, primary_concern="stress"), "I'm feeling stressed about work"),
            (AnalysisResult(risk_level=RiskLevel.MODERATE, primary_concern="anxiety"), "I'm feeling anxious and overwhelmed"),
            (AnalysisResult(risk_level=RiskLevel.HIGH, primary_concern="self-harm"), "I want to hurt myself"),
        ]

        for analysis, message in scenarios:
            assessment = checker.assess(analysis, message)
            assert assessment.risk_stage in {"관심", "주의", "위험"}
