...
FOLLOWUP_TEMPLATES = {
    ...
    IntentLabel.LOW_MOOD_SUPPORT: "오늘 가장 버겁게 느껴진 건 몸의 피곤함에 가까웠나요, 마음의 무거움에 가까웠나요?",
    IntentLabel.STRESS_SUPPORT: "지금 제일 부담되는 건 해야 할 일이 많다는 느낌인가요, 아니면 어디서부터 시작할지 모르겠는 느낌인가요?",
    IntentLabel.WORK_OR_STUDY_STRESS: "지금 제일 부담되는 건 시작하기 어려움인가요, 끝내야 한다는 압박인가요?",
    IntentLabel.RELATIONSHIP_STRESS: "그 관계에서 가장 힘든 부분은 말이 통하지 않는 느낌인가요, 혼자 감당하는 느낌인가요?",
...
def generate_followup_question(
    ...
    if previous and not _matches_avoid_topics(previous, avoid_topics):
        return previous
